...
vert.cols <- c(rep("tan3",4),"orchid3","tan3",rep("orchid3",2),"darkseagreen","tan3",rep("gray70",5))

# Vertex layout
vert.lay <- as.matrix(read.csv(paste0(W_D,"/data/westend_causal_web_coords.csv"),header = F))

plot(g,asp=0.8, vertex.label.color="black", vertex.label.cex=0.8, edge.arrow.size=0.6,vertex.label.font=2, vertex.size=25,edge.lty=2,edge.color="gray80",vertex.frame.color=F,vertex.color=vert.cols,main="",layout=vert.lay,edge.curved=F)
...
* Macrocystis itself (a primary producer) and *Cystoseira* are central to dynamics, having the most total connections, followed by the two urchin species, the gastropod *Megastraea undosa*, and the understory kelps.
...
We choose a subset of species which all show causal links to each other. We can do this with a subset of our cross-mapping exercise from Step 4. We include the NPGO index, SST, and maximum wave height as physical variables that map to almost all our variables.
...
cohesive.CCM.plot <- xmap_subset(c("mac","purp","cys","wavy","npgo.mean","lam","red","waves.max","sst.mean"))
cohesive.CCM.plot
...
This is almost a fully resolved causation web, in that every variable causally links to almost all other variables. We could think of it as a coherent subset of the overall kelp forest dynamic system, and we can be confident that these species all interact closely and therefore that a multivariate manifold using these species is warranted. With three physical variable, three species of algae, and three species of invertebrates, it is also a good microcosm of intermediate complexity, representative of the larger kelp forest community.
...
species <- c("mac","purp","cys","wavy","lam","red")
lib.vars <- c(species,"npgo.mean","waves.max","sst.mean")
