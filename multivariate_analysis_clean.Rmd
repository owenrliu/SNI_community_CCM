...
#### Maximal Difference, West End sites

```{r west end test}
westend.sites <- c("2_10L", '2_22L', "2_32R", "2_39L", "2_45L", "3_10R" ,"3_22R", "3_32L", "3_39R", "3_45L")
westend <- multiblock %>% filter(site %in% westend.sites) %>% select(period,site,one_of(lib.vars))
...
# Find best West End theta
test <- block_lnlp(westend,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),columns=lib.vars,target_column="mac",num_neighbors=0,method="s-map",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8),save_smap_coefficients = F, silent=T)

# Run best S-map and save coeffs and model output
test2 <- block_lnlp(westend,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),columns=lib.vars,target_column="mac",num_neighbors=0,method="s-map",theta = 4,save_smap_coefficients = T, silent=T)[[1]]

# find error over time
test3 <- test2$model_output %>% mutate(abs=abs(pred-obs)) %>% bind_cols(westend)
test4 <- test3 %>% group_by(period) %>% summarise(mae=mean(abs,na.rm=T))
