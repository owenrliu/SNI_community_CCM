...
for(i in 1:length(species)) {
  plot(westend.barrens$npgo.mean,westend.coeffs.barrens$mac[,species[i]],
       xlab="NPGO (nutrients)",ylab="Interaction Strength",main=sppnames[i],ylim=c(-2,2))
  abline(h=0,lty=3)
  abline(lm(westend.coeffs.barrens$mac[,species[i]]~westend.barrens$npgo.mean),col="blue",lty=2)
  barrens.ints.coef.pvals[i,"coef"] <- summary(lm(westend.coeffs.barrens$mac[,species[i]]~westend.barrens$npgo.mean))$coefficients[,1][2]
  barrens.ints.coef.pvals[i,"p"] <- summary(lm(westend.coeffs.barrens$mac[,species[i]]~westend.barrens$npgo.mean))$coefficients[,4][2]
  title("Interactions with Macrocystis (>1m) in Barrens State", outer=T)
}
...
for(i in 1:length(species)) {
  plot(westend.forests$npgo.mean,westend.coeffs.forests$mac[,species[i]],
       xlab="NPGO (nutrients)",ylab="Interaction Strength",main=sppnames[i],ylim=c(-2,2))
  abline(h=0,lty=3)
  abline(lm(westend.coeffs.forests$mac[,species[i]]~westend.forests$npgo.mean),col="blue",lty=2)
  forests.ints.coef.pvals[i,"coef"] <- summary(lm(westend.coeffs.forests$mac[,species[i]]~westend.forests$npgo.mean))$coefficients[,1][2]
  forests.ints.coef.pvals[i,"p"] <- summary(lm(westend.coeffs.forests$mac[,species[i]]~westend.forests$npgo.mean))$coefficients[,4][2]
  title("Interactions with Macrocystis (>1m) in Forested State", outer=T)
}
