...
```{r site cross prediction plots, include=F,message=F}
...
![Predictability across SNI monitoring sites by species using simplex projection. Gray indicates a non-significant signal, and warmer colors indicated higher correlation between forecasts and observations.](x_site_pred.png)

As we can see from the plots, some sites are not predictable for some species (e.g., Site 7 for juvenile *Macrocystis*, or Site 5 for the wavy turban snail *Megastraea undosa*). However, there are no sites which are universally unpredictable or useless as library sites across all species (although Site 7 is not very predictable for most species, and may be worth revisiting later).  For now, we can feel confident in using all the available data for these species.
...
plot(g,asp=0.8, vertex.label.color="black", vertex.label.cex=0.8, edge.arrow.size=0.6,vertex.label.font=2, vertex.size=25,edge.lty=2,edge.color="gray80",vertex.frame.color=F,vertex.color=vert.cols,main="",edge.curved=F,layout=vert.lay)

legend(x=-1, y=-1, c("Brown Algae","Echinoderms", "Gastropods","Physical Forcing"), pch=21, pt.bg=c("tan3","orchid3","darkseagreen","gray70"),pt.cex=2, cex=1, bty="n", ncol=1,text.col="white")
```
...
cohesive.CCM.plot <- xmap_subset(c("mac","purp","ymac","lam","wavy","npgo.mean","pter","red"))
cohesive.CCM.plot
...
## Means and SDs
effect.means <- function(df) {
  out <- df %>% group_by(effect,plotting) %>% 
    na.omit() %>% 
    summarise(mean=mean(value),sd=sd(value)) %>%
    mutate(upper=mean+sd,lower=mean-sd)
  return(out)
}
coh.long.means <- lapply(coh.long,effect.means)
means.plot <- function(df) {
  plot.out <- ggplot(df,aes(x=plotting,y=mean,ymin=lower,ymax=upper)) +
...
effect.means.plots <- lapply(coh.long.means,means.plot)
```
...
### Macrocystis multivariate model, for nonlinear reference
temp <- block_lnlp(multiblock,lib=as.matrix(segs),pred=as.matrix(segs),columns=lib.vars,target_column="mac",num_neighbors=0,method="s-map",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8),save_smap_coefficients = F, silent=T)
plot(temp$theta,temp$rho,main="S-map for Multivariate Macrocystis Model",xlab=expression(paste("Nonlinearity (",theta,")")), ylab=expression(paste("Skill, ",rho)),type="l")

### Example with effect of macrocystis on laminaria in the presence of nutrients
par(mfrow=c(2,1),mar=c(4,4,1,2)+0.1)
quant_reg_plot(multiblock$npgo.mean,multi.mods[['lam']]$smap_coefficients[,'mac'], xlab="",
               ylab=expression(paste(partialdiff,"Lam/", partialdiff,"Mac")))
abline(lm(multi.mods[['lam']]$smap_coefficients[,'mac']~multiblock$npgo.mean),col="blue",lty=2)
quant_reg_plot(multiblock$npgo.mean,multi.mods[['lam']]$smap_coefficients[,'purp'],
               xlab="Standardized NPGO Index",ylab=expression(paste(partialdiff,"Lam/", partialdiff,"Purple")))
abline(lm(multi.mods[['lam']]$smap_coefficients[,'purp']~multiblock$npgo.mean),col="blue",lty=2)
par(mar=c(5,4,4,2)+0.1)
```

Means across all segments by time. Takes a dataframe of model output or observations and finds means over time

```{r seg means}
multiblock.means <- multiblock %>% group_by(period) %>% summarise_each(funs(m=mean(.,na.rm=T),sd=sd(.,na.rm=T)),one_of(lib.vars))

test <- select(multiblock.means,period,purp_m,purp_sd,npgo.mean_m,pter_m,pter_sd,mac_m,mac_sd)

ggplot(test,aes(x=period)) +geom_line(aes(y=purp_m))+
  geom_pointrange(aes(y=purp_m,ymin=purp_m-purp_sd,ymax=purp_m+purp_sd),col="purple")+
  geom_line(aes(y=pter_m))+
  geom_pointrange(aes(y=pter_m,ymin=pter_m-pter_sd,ymax=pter_m+pter_sd),col="orange")+
  geom_line(aes(y=mac_m))+
  geom_pointrange(aes(y=mac_m,ymin=mac_m-mac_sd,ymax=mac_m+mac_sd),col="darkgreen")+
  geom_line(aes(y=npgo.mean_m),color="blue")
...
#### Split Models by Island Area

SNI has 4 major groupings of sites: The West End (sites 2 and 3), Dutch Harbor along the southern coast (site 4 and 5), Daytona along the southeast (Site 6), and Nav Fac on the north side (site 1). In this step, we fit the same models as above and look for differences between sites.
...
plot(westend$red,westend$mac,pch=19)
abline(v=-0.5,lty=2)
...
# Data frame to hold optimal theta, and associated rho and mean absolute error (MAE) associated with the chosen model
westend.thetas <- data_frame(species=species,theta=numeric(length(species)),rho=numeric(length(species)),mae=numeric(length(species)))

## Westend multivariate models
westend.mods <- list()
westend.coeffs <- list()
westend.coeffs.long <- list()
westend.box <- list()
...
for (i in 1:7) {
  ...
  # Run multivariate S-maps with varying theta
  temp<-block_lnlp(westend,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),columns=westend.lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8),save_smap_coefficients = F, silent=T)
  
  # Find optimal theta based on best predictive skill
  opt.stats <- temp %>% filter(rho==max(rho))
  westend.thetas$theta[i] <- opt.stats$theta
  westend.thetas$rho[i] <- opt.stats$rho
  westend.thetas$mae[i] <- opt.stats$mae
  
  # Save S-map coefficients for the optimal model
  best_model <- block_lnlp(westend,lib=as.matrix(westend.segs),pred=as.matrix(westend.segs),columns=westend.lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = opt.stats$theta,save_smap_coefficients = T, silent=T)
  
  # rename s-map coefficients to variable names for later reference
  westend.coeffs[[spp]] <- best_model[[1]]$smap_coefficients %>% as.data.frame()
  names(westend.coeffs[[spp]]) <- c(westend.lib.vars,"const")
  westend.coeffs[[spp]]$state <- westend$state
  westend.coeffs[[spp]]$period <- westend$period
  westend.mods[[spp]] <- best_model[[1]]
  
  # long form for plotting
  westend.coeffs.long[[spp]] <- westend.coeffs[[spp]] %>%
  ...
  # box plot of interactions
  sppname <- fullkey$long[match(spp,fullkey$short)]
  westend.box[[spp]]<- ggplot(westend.coeffs.long[[spp]],aes(x=pred.name,y=value,fill=state))+
  geom_boxplot() +
  geom_hline(yintercept=0,linetype=2,col="gray40")+
  xlab("Interacting Variable") +
  ylab("Coefficient") +
...
westend_mean_ints <- list()
westend_mean_ints_plot <- list()
for(i in 1:length(species)) {
  spp <- species[i]
  sppname <- fullkey$long[match(spp,fullkey$short)]
  westend_mean_ints[[spp]] <- westend.coeffs.long[[spp]] %>% 
    group_by(period,effect) %>% 
    summarise(meanint=mean(value,na.rm=T),sdint=sd(value,na.rm=T))%>%
    mutate(lower=meanint-sdint,upper=meanint+sdint)
  westend_mean_ints_plot[[spp]] <- ggplot(westend_mean_ints[[spp]],aes(x=period,y=meanint))+
...
westend.barrens <- filter(westend,state=="barren")
westend.forests <- filter(westend,state=="forest")
westend.coeffs.barrens <- lapply(westend.coeffs,function(x) filter(x,state=="barren"))
westend.coeffs.forests <- lapply(westend.coeffs,function(x) filter(x,state=="forest"))
...
for(i in 1:length(species)) {
  plot(westend.barrens$npgo.mean,westend.coeffs.barrens$lam[,species[i]],
       xlab="NPGO (nutrients)",ylab="Interaction Strength",main=sppnames[i],ylim=c(-2,2))
  abline(h=0,lty=3)
  abline(lm(westend.coeffs.barrens$lam[,species[i]]~westend.barrens$npgo.mean),col="blue",lty=2)
  barrens.ints.coef.pvals[i,"coef"] <- summary(lm(westend.coeffs.barrens$lam[,species[i]]~westend.barrens$npgo.mean))$coefficients[,1][2]
  barrens.ints.coef.pvals[i,"p"] <- summary(lm(westend.coeffs.barrens$lam[,species[i]]~westend.barrens$npgo.mean))$coefficients[,4][2]
...
for(i in 1:length(species)) {
  plot(westend.forests$npgo.mean,westend.coeffs.forests$lam[,species[i]],
       xlab="NPGO (nutrients)",ylab="Interaction Strength",main=sppnames[i],ylim=c(-2,2))
  abline(h=0,lty=3)
  abline(lm(westend.coeffs.forests$lam[,species[i]]~westend.forests$npgo.mean),col="blue",lty=2)
  forests.ints.coef.pvals[i,"coef"] <- summary(lm(westend.coeffs.forests$lam[,species[i]]~westend.forests$npgo.mean))$coefficients[,1][2]
  forests.ints.coef.pvals[i,"p"] <- summary(lm(westend.coeffs.forests$lam[,species[i]]~westend.forests$npgo.mean))$coefficients[,4][2]
  title("Interactions with lamrocystis (>1m) in Forested State", outer=T)
}
forests.ints.coef.pvals

par(mfrow=c(1,1))
# Does model error go up with NPGO? Nope
plot(westend$npgo.mean,abs(westend.mods$mac$model_output$obs-westend.mods$mac$model_output$pred),
     xlab="NPGO",ylab="Absolute error")

# Does actual variance in density go up with NPGO? Maybe
westend.npgo.var <- westend.mods$mac$model_output %>% 
  bind_cols(data_frame(period=lead(westend$period,1))) %>% 
  bind_cols(data_frame(npgo.mean=lead(westend$npgo.mean,1))) %>%
  group_by(npgo.mean) %>%
  summarise(obs.var = sd(obs,na.rm=T),pred.var = sd(pred,na.rm=T))
plot(westend.npgo.var$npgo.mean,westend.npgo.var$obs.var,xlab="NPGO",ylab="SD of Macrocystis Density")
abline(lm(westend.npgo.var$obs.var~westend.npgo.var$npgo.mean),lty=2)

## t-test differences between barrens and forests
ttest_ints <- function(spp) {
  out <- data_frame(spp=rep(spp,length(lib.vars)),
                    effect=character(length(lib.vars)),
                    barrens_sig=character(length(lib.vars)),
                    forests_sig=character(length(lib.vars)),
                    barrens_int=numeric(length(lib.vars)),
                    forest_int=numeric(length(lib.vars)),
                    p=numeric(length(lib.vars)))
  for(i in 1:length(lib.vars)) {
    eff <- lib.vars[i]
    temp <- t.test(westend.coeffs.barrens[[spp]][,eff],westend.coeffs.forests[[spp]][,eff])
    barrens.sig <- ifelse(t.test(westend.coeffs.barrens[[spp]][,eff])$p.value<0.05,"Yes","No")
    forest.sig <- ifelse(t.test(westend.coeffs.forests[[spp]][,eff])$p.value<0.05,"Yes","No")
    stats <- as.numeric(round(c(temp$estimate[1],temp$estimate[2],temp$p.value),4))
    out[i,1:4] <- c(spp,eff,barrens.sig,forest.sig)
    out[i,5:7] <-stats
  }
  return(out)
}
diff.tests <- lapply(species,ttest_ints)
names(diff.tests) <- species
...
# Data frame to hold optimal theta, and associated rho and mean absolute error (MAE) associated with the chosen model
daytona.thetas <- data_frame(species=species,theta=numeric(length(species)),rho=numeric(length(species)),mae=numeric(length(species)))

## daytona multivariate models
daytona.mods <- list()
daytona.coeffs <- list()
daytona.coeffs.long <- list()
daytona.box <- list()
...
for (i in 1:7) {
  ...
  # Run multivariate S-maps with varying theta
  temp<-block_lnlp(daytona,lib=as.matrix(daytona.segs),pred=as.matrix(daytona.segs),columns=lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8),save_smap_coefficients = F, silent=T)
  
  # Find optimal theta based on best predictive skill
  opt.stats <- temp %>% filter(rho==max(rho))
  daytona.thetas$theta[i] <- opt.stats$theta
  daytona.thetas$rho[i] <- opt.stats$rho
  daytona.thetas$mae[i] <- opt.stats$mae
  
  # Save S-map coefficients for the optimal model
  best_model <- block_lnlp(daytona,lib=as.matrix(daytona.segs),pred=as.matrix(daytona.segs),columns=lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = opt.stats$theta,save_smap_coefficients = T, silent=T)
  
  # rename s-map coefficients to variable names for later reference
  daytona.coeffs[[spp]] <- best_model[[1]]$smap_coefficients %>% as.data.frame()
  names(daytona.coeffs[[spp]]) <- c(lib.vars,"const")
  daytona.coeffs[[spp]]$period <- daytona$period
  daytona.mods[[spp]] <- best_model[[1]]
  
  # long form for plotting
  daytona.coeffs.long[[spp]] <- daytona.coeffs[[spp]] %>%
  ...
  # box plot of interactions
  sppname <- fullkey$long[match(spp,fullkey$short)]
  daytona.box[[spp]]<- ggplot(daytona.coeffs.long[[spp]],aes(x=pred.name,y=value,fill=effect))+
    geom_boxplot() +
    geom_hline(yintercept=0,linetype=2,col="gray40")+
    xlab("Interacting Variable") +
    ylab("Coefficient") +
    guides(fill="none")+
    ggtitle(paste("Interactive Effects on Dynamics of ",sppname,", Daytona"))
}

daytona.ints.coef.pvals <- data_frame(species=species,coef=numeric(length(species)),p=numeric(length(species)))
par(mfrow=c(2,2),oma=c(0,0,2,0))
for(i in 1:length(species)) {
  plot(daytona$npgo.mean,daytona.coeffs$mac[,species[i]],
       xlab="NPGO (nutrients)",ylab="Interaction Strength",main=sppnames[i],ylim=c(-1,1))
  abline(h=0,lty=3)
  abline(lm(daytona.coeffs$mac[,species[i]]~daytona$npgo.mean),col="blue",lty=2)
  daytona.ints.coef.pvals[i,"coef"] <- summary(lm(daytona.coeffs$mac[,species[i]]~daytona$npgo.mean))$coefficients[,1][2]
  daytona.ints.coef.pvals[i,"p"] <- summary(lm(daytona.coeffs$mac[,species[i]]~daytona$npgo.mean))$coefficients[,4][2]
  title("Interactions with Macrocystis (>1m) across NPGO", outer=T)
}

# Mean interactions over time (across spatial replicates)
daytona_mean_ints <- list()
daytona_mean_ints_plot <- list()
for(i in 1:length(species)) {
  spp <- species[i]
  sppname <- fullkey$long[match(spp,fullkey$short)]
  daytona_mean_ints[[spp]] <- daytona.coeffs.long[[spp]] %>% 
    group_by(period,effect) %>% 
    summarise(meanint=mean(value,na.rm=T),sdint=sd(value,na.rm=T))%>%
    mutate(lower=meanint-sdint,upper=meanint+sdint)
  daytona_mean_ints_plot[[spp]] <- ggplot(daytona_mean_ints[[spp]],aes(x=period,y=meanint))+
    geom_hline(yintercept=0,linetype=2)+
    geom_line()+
    geom_pointrange(aes(ymin=lower,ymax=upper),size=0.3)+
    xlab("Period")+ylab("Interaction Strength")+
...
# Data frame to hold optimal theta, and associated rho and mean absolute error (MAE) associated with the chosen model
dutch.thetas <- data_frame(species=species,theta=numeric(length(species)),rho=numeric(length(species)),mae=numeric(length(species)))

## dutch multivariate models
dutch.mods <- list()
dutch.coeffs <- list()
dutch.coeffs.long <- list()
dutch.box <- list()
...
for (i in 1:7) {
  ...
  # Run multivariate S-maps with varying theta
  temp<-block_lnlp(dutch,lib=as.matrix(dutch.segs),pred=as.matrix(dutch.segs),columns=lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8),save_smap_coefficients = F, silent=T)
  
  # Find optimal theta based on best predictive skill
  opt.stats <- temp %>% filter(rho==max(rho))
  dutch.thetas$theta[i] <- opt.stats$theta
  dutch.thetas$rho[i] <- opt.stats$rho
  dutch.thetas$mae[i] <- opt.stats$mae
  
  # Save S-map coefficients for the optimal model
  best_model <- block_lnlp(dutch,lib=as.matrix(dutch.segs),pred=as.matrix(dutch.segs),columns=lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = opt.stats$theta,save_smap_coefficients = T, silent=T)
  
  # rename s-map coefficients to variable names for later reference
  dutch.coeffs[[spp]] <- best_model[[1]]$smap_coefficients %>% as.data.frame()
  names(dutch.coeffs[[spp]]) <- c(lib.vars,"const")
  dutch.coeffs[[spp]]$period <- dutch$period
  dutch.mods[[spp]] <- best_model[[1]]
  
  # long form for plotting
  dutch.coeffs.long[[spp]] <- dutch.coeffs[[spp]] %>%
    gather(key=effect,value=value,-period,na.rm=T) %>%
    left_join(fullkey, by=c("effect"="short")) %>%
    mutate(pred.name=fullkey$plotting[match(effect,fullkey$short)]) %>%
    group_by(effect)
  ...
  # box plot of interactions
  sppname <- fullkey$long[match(spp,fullkey$short)]
  dutch.box[[spp]]<- ggplot(dutch.coeffs.long[[spp]],aes(x=pred.name,y=value,fill=effect))+
    geom_boxplot() +
    geom_hline(yintercept=0,linetype=2,col="gray40")+
    xlab("Interacting Variable") +
    ylab("Coefficient") +
    guides(fill="none")+
    ggtitle(paste("Interactive Effects on Dynamics of ",sppname,", Dutch Harbor"))
}

# Mean interactions over time (across spatial replicates)
dutch_mean_ints <- list()
dutch_mean_ints_plot <- list()
for(i in 1:length(species)) {
  spp <- species[i]
  sppname <- fullkey$long[match(spp,fullkey$short)]
  dutch_mean_ints[[spp]] <- dutch.coeffs.long[[spp]] %>% 
    group_by(period,effect) %>% 
    summarise(meanint=mean(value,na.rm=T),sdint=sd(value,na.rm=T))%>%
    mutate(lower=meanint-sdint,upper=meanint+sdint)
  dutch_mean_ints_plot[[spp]] <- ggplot(dutch_mean_ints[[spp]],aes(x=period,y=meanint))+
    geom_hline(yintercept=0,linetype=2)+
    geom_line()+
    geom_pointrange(aes(ymin=lower,ymax=upper),size=0.3)+
    xlab("Period")+ylab("Interaction Strength")+
...
# Data frame to hold optimal theta, and associated rho and mean absolute error (MAE) associated with the chosen model
navfac.thetas <- data_frame(species=species,theta=numeric(length(species)),rho=numeric(length(species)),mae=numeric(length(species)))

## navfac multivariate models
navfac.mods <- list()
navfac.coeffs <- list()
navfac.coeffs.long <- list()
navfac.box <- list()
...
for (i in 1:7) {
  ...
  # Run multivariate S-maps with varying theta
  temp<-block_lnlp(navfac,lib=as.matrix(navfac.segs),pred=as.matrix(navfac.segs),columns=lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8),save_smap_coefficients = F, silent=T)
  
  # Find optimal theta based on best predictive skill
  opt.stats <- temp %>% filter(rho==max(rho))
  navfac.thetas$theta[i] <- opt.stats$theta
  navfac.thetas$rho[i] <- opt.stats$rho
  navfac.thetas$mae[i] <- opt.stats$mae
  
  # Save S-map coefficients for the optimal model
  best_model <- block_lnlp(navfac,lib=as.matrix(navfac.segs),pred=as.matrix(navfac.segs),columns=lib.vars,target_column=spp,num_neighbors=0,method="s-map",theta = opt.stats$theta,save_smap_coefficients = T, silent=T)
  
  # rename s-map coefficients to variable names for later reference
  navfac.coeffs[[spp]] <- best_model[[1]]$smap_coefficients %>% as.data.frame()
  names(navfac.coeffs[[spp]]) <- c(lib.vars,"const")
  navfac.coeffs[[spp]]$period <- navfac$period
  navfac.mods[[spp]] <- best_model[[1]]
  
  # long form for plotting
  navfac.coeffs.long[[spp]] <- navfac.coeffs[[spp]] %>%
    gather(key=effect,value=value,-period,na.rm=T) %>%
    left_join(fullkey, by=c("effect"="short")) %>%
    mutate(pred.name=fullkey$plotting[match(effect,fullkey$short)]) %>%
    group_by(effect)
  ...
  # box plot of interactions
  sppname <- fullkey$long[match(spp,fullkey$short)]
  navfac.box[[spp]]<- ggplot(navfac.coeffs.long[[spp]],aes(x=pred.name,y=value,fill=effect))+
    geom_boxplot() +
    geom_hline(yintercept=0,linetype=2,col="gray40")+
    xlab("Interacting Variable") +
    ylab("Coefficient") +
    guides(fill="none")+
    ggtitle(paste("Interactive Effects on Dynamics of ",sppname,", Nav Fac"))
}

navfac.ints.coef.pvals <- data_frame(species=species,coef=numeric(length(species)),p=numeric(length(species)))
par(mfrow=c(2,2),oma=c(0,0,2,0))
for(i in 1:length(species)) {
  plot(navfac$npgo.mean,navfac.coeffs$mac[,species[i]],
       xlab="NPGO (nutrients)",ylab="Interaction Strength",main=sppnames[i],ylim=c(-1,1))
  abline(h=0,lty=3)
  abline(lm(navfac.coeffs$mac[,species[i]]~navfac$npgo.mean),col="blue",lty=2)
  navfac.ints.coef.pvals[i,"coef"] <- summary(lm(navfac.coeffs$mac[,species[i]]~navfac$npgo.mean))$coefficients[,1][2]
  navfac.ints.coef.pvals[i,"p"] <- summary(lm(navfac.coeffs$mac[,species[i]]~navfac$npgo.mean))$coefficients[,4][2]
  title("Interactions with macrocystis (>1m) across NPGO", outer=T)
}

# Mean interactions over time (across spatial replicates)
navfac_mean_ints <- list()
navfac_mean_ints_plot <- list()
for(i in 1:length(species)) {
  spp <- species[i]
  sppname <- fullkey$long[match(spp,fullkey$short)]
  navfac_mean_ints[[spp]] <- navfac.coeffs.long[[spp]] %>% 
    group_by(period,effect) %>% 
    summarise(meanint=mean(value,na.rm=T),sdint=sd(value,na.rm=T))%>%
    mutate(lower=meanint-sdint,upper=meanint+sdint)
  navfac_mean_ints_plot[[spp]] <- ggplot(navfac_mean_ints[[spp]],aes(x=period,y=meanint))+
    geom_hline(yintercept=0,linetype=2)+
    geom_line()+
    geom_pointrange(aes(ymin=lower,ymax=upper),size=0.3)+
    xlab("Period")+ylab("Interaction Strength")+
...
Dutch Harbor looks super different than other sites (much lower predictability). Let's investigate.

```{r dutch data explo}

dutch.simp.list <- list()
# we go back to our original study spp
dutch_spp <- c("pat","red","purp","wavy","paras","cys","lam","eis","pter","mac","ymac")
dutch <- benthdat.block %>% left_join(phys.dat.norm,by="period") %>% filter(site %in% dutch.sites)

# Run simplex projection for each species, and plot output
par(mfrow=c(3,2))
for(i in 1:length(dutch_spp)) {
  spp <- dutch_spp[i]
  dat <- dutch %>% select(matches(spp)) %>% as.data.frame()
  out <- simplex(as.numeric(dat[,1]),lib=as.matrix(dutch.segs),E=2:15,silent=T)
  dutch.simp.list[[spp]] <- out
...
# Not promising diagnostics... can CCM give us a clue?

# CCM ############

# Rows hold predicted variables, columns hold forcing variables. There are more columns than rows since the physical variables are included as potential forcing factors.
dutch <- dutch %>% select(-pis)
n_col <- dim(dutch)[2]-2
n_row <- dim(dutch)[2]-7
col_names <- colnames(dutch)[3:(n_col+2)]
row_names <- colnames(dutch)[3:(n_row+2)]
dutch_xmap_mat <- array(NA,dim=c(n_row,n_col),dimnames=list(row_names,col_names))
...
for(i in 1:n_row) {
  for(j in 1:n_col) {
    if(i != j) {
      ...
      temp <- ccm(dutch,lib=as.matrix(dutch.segs),pred=as.matrix(dutch.segs),E=tempE,lib_column= 2+i,target_column = 2+j,lib_sizes = c(10,500),num_samples=100,replace=T,silent=T)
      # mean rho at library size 500
      rhomean <- temp %>% filter(lib_size==500) %>% ccm_means()
      dutch_xmap_mat[i,j] <- rhomean$rho
...
# keep only signficant cross-mappings
dutch_xmap_mat <-dutch_xmap_mat*dutch.ptot.mat %>% as.data.frame()
dutch_xmap_rast <- dutch_xmap_mat %>% 
  mutate(predictee=row.names(dutch_xmap_mat)) %>% 
  gather(key=predictor,value=rho,cys:sst.mean)

# If rho is zero, replace with NA (no significant causal signal)
dutch_xmap_rast$rho[dutch_xmap_rast$rho==0] <- NA

# Names for plotting to distinguish biological and physical variables
dutch_xmap_rast <- dutch_xmap_rast %>% 
  mutate(force.name=fullkey$plotting[match(predictor,fullkey$short)]) %>%
  mutate(pred.name=fullkey$plotting[match(predictee,fullkey$short)])

## plot
dutch_xmap_all_plot <- ggplot(dutch_xmap_rast,aes(x=force.name,y=pred.name,fill=rho)) +
  geom_raster() +
  scale_fill_gradient(low = "#9AFF9A", high = "#548B54", space = "Lab", na.value = "grey50", 
      guide = "colourbar",name=expression(paste(rho, "(skill)"))) +
  ggtitle("Kelp Forest Convergent Cross Mapping") +
...
```{r sample plots}
### Pretty dynamic interaction plot
test5 <- multi.mods$pter$smap_coefficients %>% as.data.frame() %>% filter(!is.na(mac)) %>% mutate(time=row_number())
dyn.pter.plot <- ggplot(test5,aes(x=time))+
  geom_line(aes(y=lam,col="Laminaria"),lwd=1)+
  geom_line(aes(y=mac,col="Macrocystis"),lwd=1)+
  geom_line(aes(y=purp,col="S. purpuratus"),lwd=1)+
  xlim(0,200)+
  scale_colour_manual(name="Effect(partial derivative)",labels=c("Laminaria","Macrocystis","S. purpuratus"),values=c("firebrick","chartreuse4","darkorchid4"))+
  ggtitle("Dynamic Interaction with Pterygophora californica")+
  theme(legend.key = element_blank(),legend.justification=c(0,1), legend.position=c(0,1),
        panel.background=element_rect(fill="white",color="black"),aspect.ratio=0.56)+
  geom_hline(yintercept=0,linetype=2,color="gray40")+
  ylab("estimated interaction strength")
dyn.pter.plot
ggsave("pterygophora dynamic plot.png")

## Pterygophora observations and predictions
test6 <- multi.mods$pter$model_output %>% as.data.frame() %>% filter(!is.na(obs)) %>%
  select(-pred_var) %>% mutate(pred=lead(pred,1),abs=abs(obs-pred)) %>%
  gather(key=type,value=value,obs:pred)
pter.obs.plot <- ggplot(test6,aes(x=time,y=value,colour=type))+
  geom_line(lwd=1)+
  xlim(650,750)+ylim(-1,4)+
  ggtitle("Pterygophora californica Observations and Predictions")+
  theme(legend.key = element_blank(),legend.justification=c(0,1), legend.position=c(0,1),
        panel.background=element_rect(fill="white",color="black"),aspect.ratio=0.56)+
  geom_hline(yintercept=0,linetype=2,color="gray40")+
  scale_color_discrete(name="",labels=c("Observed","Predicted"))+
