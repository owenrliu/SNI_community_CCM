...
multi_vars <- colnames(multiblock)[2:ncol(multiblock)]
...
for(i in 1:15) {
  ...
  for(j in 1:length(poss_vars)) {
    ...
    temp <- block_lnlp(multiblock,lib=rnd_lib_segs,pred=rnd_pred_segs,columns=temp_vars,target_column = "mac")
    out.rho[j] <- temp$rho
  }
...
# Plot results
plot(1:15,selected.vars$cum.rho,type="l",main="Forward Model Selection for Macrocystis Dynamics",xlab="Model Selection Step (= Number of Variables Included)",ylab=expression(paste("Model Predictive Skill, ",rho)))
```
