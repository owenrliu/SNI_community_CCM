...
```{r ccm mac mei}
mac_xmap_mei <- ccm(CCM.block.phys,E=bestE["mac"],lib=as.matrix(segs),pred=as.matrix(segs),lib_column="mac",target_column="mei.mean",lib_sizes=c(seq(5,55,by=5),seq(55,1000,by=10)),random_libs=T,silent=T)

mei_xmap_mac <- ccm(CCM.block.phys,E=bestE["mei.mean"],lib=as.matrix(segs),pred=as.matrix(segs),lib_column = "mei.mean", target_column = "mac",lib_sizes=c(seq(5,55,by=5),seq(55,1000,by=10)),random_libs=T,silent=T)
...
### PDO ###
mac_xmap_pdo <- ccm(CCM.block.phys,E=bestE["mac"],lib_column="mac",target_column="pdo.mean",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)

pdo_xmap_mac <- ccm(CCM.block.phys,E=bestE["pdo.mean"],lib_column = "pdo.mean", target_column = "mac",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)
...
### NPGO ###
mac_xmap_npgo <- ccm(CCM.block.phys,E=bestE["mac"],lib_column="mac",target_column="npgo.mean",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)

npgo_xmap_mac <- ccm(CCM.block.phys,E=bestE["npgo.mean"],lib_column = "npgo.mean", target_column = "mac",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)
...
### Hs ###
mac_xmap_waves <- ccm(CCM.block.phys,E=bestE["mac"],lib_column="mac",target_column="waves.max",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)

waves_xmap_mac <- ccm(CCM.block.phys,E=bestE["waves.max"],lib_column = "waves.max", target_column = "mac",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)
...
### SST ###
mac_xmap_sst <- ccm(CCM.block.phys,E=bestE["mac"],lib_column="mac",target_column="sst.mean",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)

sst_xmap_mac <- ccm(CCM.block.phys,E=bestE["sst.mean"],lib_column = "sst.mean", target_column = "mac",lib_sizes=c(seq(5,70,by=5)),random_libs=T,silent=T)
...
```
We can re-run the previous all-species cross-mapping to build physical variables into our causation web:

```{r ccm all species and physical vars}
ncol <- dim(CCM.block.phys)[2]-1
col.names <- colnames(CCM.block.phys)[2:(ncol+1)]
xmap_mat <- array(NA,dim=c(ncol,ncol),dimnames=list(col.names,col.names))
...
for(i in 1:ncol) {
  for(j in 1:ncol) {
    if(i != j) {
      # remember, we use the best embedding dimension for the library variable (the predicted variable, j)
      tempE=bestE[col.names[j]]
      temp <- ccm(CCM.block.phys,lib=rnd_lib_segs,pred=rnd_pred_segs,E=tempE,lib_column= 1+i,target_column = 1+j,lib_sizes = c(10,500),replace=T,silent=T)
      # mean rho at library size 500
      rhomean <- temp %>% filter(lib_size==500) %>% ccm_means()
      xmap_mat[i,j] <- rhomean$rho
...
```{r CCM raster, echo=F}
xmap_rast_with_phys <- as.data.frame(xmap_mat*ptot.mat) %>% 
  mutate(predictee=row.names(xmap_mat)) %>% 
...
multiblock <- dat.norm.block %>%
  ...
  #purp
  mutate(purp.lag1=lag(purp,1),purp.lag2=lag(purp,2)) %>%
  ungroup() %>%
  select(-site)
...
mac.multi <- block_lnlp(multiblock,lib=rnd_lib_segs,pred=rnd_pred_segs,columns=c("lam","lam.lag1","lam.lag2","pter","pter.lag1","pter.lag2","purp","purp.lag1","purp.lag2"),target_column = "mac",stats_only = F,silent=T)

mac.multi.pter[[1]]$stats$rho

mac.multi.purp[[1]]$stats$rho
mac.multi[[1]]$stats$rho
```