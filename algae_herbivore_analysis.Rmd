...
ptm <- proc.time()
mac.back.vars <- names(multiblock[2:17])

mac.back.mod <- data_frame(ex.var=character(16),rho=numeric(16),theta=numeric(16))
...
for(i in 2:16) {
  ...
  for(j in 1:length(poss_vars)) {
    ...
    temp <- block_lnlp(multiblock,lib=rnd_lib_segs,pred=rnd_pred_segs,columns=temp_vars,target_column = "mac",theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.1, 0.5, 1, 2, 4, 6,8), num_neighbors=0,method="s-map",silent=T)
    out.rho[j] <- max(temp$rho)
    out.theta[j] <-  temp$theta[temp$rho==max(temp$rho)][1]
  }
...
plot(1:16,mac.back.mod$rho,main="Macrocystis Backwards Model Selection",xlab="Variables Excluded",ylab="Model Predictive Skill",type="l")
```
...
dyn.vars <- c(mac.back.mod$ex.var[6:16],"sst.mean")
